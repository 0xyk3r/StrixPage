...
import type { AiAttachment, AiAttachmentResp, AiMessageResp, AiSessionResp } from '@/api/ai'
...
export const useAiChatStore = defineStore(
  ...
  () => {
    ...
    async function sendMessage(content: string, attachments: (AiAttachment & { localPreview?: string })[] = []) {
      ...
      const userMsg: UiMessage = {
        ...
        content,
        thinkingContent: '',
        attachments: attachments.map((a) => ({ ...a, previewUrl: a.localPreview || '' })),
        status: 1,
        thinkingExpanded: false,
        thinkingDone: false,
        metaExpanded: false
      ...
      await streamAiMessage(
        ...
        (event) => {
          if (event.type === 'thinking') {
            ...
            targetMsg.cacheHitTokens = event.cacheHitTokens
            targetMsg.cacheWriteTokens = event.cacheWriteTokens
            targetMsg.reasoningTokens = event.reasoningTokens
