...
<template>
  ...
  >
    <n-form ref="formRef" :model="form" :rules="formRules" label-placement="left"
            label-width="auto">
      <!-- Comment (all actions) -->
      <n-form-item label="意见" path="comment">
        <n-input v-model:value="form.comment" type="textarea" :rows="3"
                 placeholder="请输入处理意见" />
      </n-form-item>
      ...
      <n-form-item v-if="action === 'return'" label="回退至" path="targetNodeId">
        <n-select v-model:value="form.targetNodeId" :options="returnNodeOptions"
                  placeholder="选择回退节点" />
      </n-form-item>
    ...
    <template #footer>
      <n-flex justify="end">
        ...
        <n-button :type="actionType" :loading="loading" @click="handleSubmit">{{ dialogTitle }}
        </n-button>
      </n-flex>
    </template>
  </n-modal>
...
async function handleSubmit() {
  ...
  try {
    const apiMap: Record<ActionType, () => Promise<any>> = {
      ...
      reject: () => workflowApi.taskReject(props.taskId, {
        comment: form.comment,
        reason: form.reason
      }),
      return: () =>
        workflowApi.taskReturn(props.taskId, {
          targetNodeId: form.targetNodeId,
          comment: form.comment
        }),
      delegate: () =>
        workflowApi.taskDelegate(props.taskId, {
          delegateId: form.delegateId,
          comment: form.comment
        }),
      countersign: () =>
        workflowApi.taskCountersign(props.taskId, {
          assigneeIds: form.assigneeIds,
          comment: form.comment
        }),
      'remove-sign': () =>
        workflowApi.taskRemoveSign(props.taskId, {
