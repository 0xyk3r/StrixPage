  ...
  >
    ...
    >
      ...
      <n-form-item v-if="action === 'return'" label="回退至" path="targetNodeId">
        <n-select
          v-model:value="form.targetNodeId"
    ...
    <template #footer>
      <n-flex justify="end">
        ...
        <n-button :type="actionType" :loading="loading" @click="handleSubmit">{{
          dialogTitle
        }}</n-button>
      </n-flex>
    </template>
...
async function handleSubmit() {
  ...
  try {
    const apiMap: Record<ActionType, () => Promise<any>> = {
      ...
      reject: () =>
        workflowApi.taskReject(props.taskId, { comment: form.comment, reason: form.reason }),
      return: () =>
