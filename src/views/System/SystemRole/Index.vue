...
// 加载字典
const systemRoleRegionPermissionTypeRef = ref([])
provide('SystemRoleRegionPermissionTypeDict', systemRoleRegionPermissionTypeRef)
onMounted(() => {
  dictsStore.getDictData('SystemRoleRegionPermissionType', systemRoleRegionPermissionTypeRef)
})
    ...
    currentMenus.push(
      h(NGrid, { xGap: 10, cols: 6 }, () => [
        h(NGi, { span: 1 }, () =>
          h(
            NTag,
            { type: colorList[colorIndex], closable: true, onClose: () => removeRoleMenu(row, menu.id) },
            () => menu.name
          )
        ),
...
const dataColumns = [
  ...
  },
  {
    ...
    width: 140,
    render(row) {
      return h(StrixTag, { value: row.regionPermissionType, dictName: 'SystemRoleRegionPermissionType' })
    }
  },
  {
    title: '操作',
...
const editData = () => {
  proxy.$refs.editDataFormRef.validate((errors) => {
    ...
    proxy.$http
      .post(`system/role/update/${editDataId}`, editDataForm.value, { operate: `修改${_baseName}` })
      .then(() => {
        initDataForm()
        getDataList()
      })
...
const removeRoleMenu = (row, menuId) => {
  proxy.$http
    .post(`system/role/remove/${row.id}/menu/${menuId}`, null, { operate: '移除角色菜单权限' })
    .then(({ data: res }) => {
      handleEditSuccessResponse(row, res.data)
...
const removeRolePermission = (row, permissionId) => {
  proxy.$http
    .post(`system/role/remove/${row.id}/permission/${permissionId}`, null, { operate: '移除角色系统权限' })
    .then(({ data: res }) => {
      handleEditSuccessResponse(row, res.data)
    })
}
...
const editRoleMenus = () => {
  ...
  const checkedIds = [
    ...proxy.$refs.editRoleMenusTree.getCheckedData().keys,
    ...proxy.$refs.editRoleMenusTree.getIndeterminateData().keys
  ]
